...
namespace MachineInterfacing
{
    public partial class frmCommInterface : Form
    {
        ...
        StringBuilder _CellDynData = new StringBuilder();
        ...
        private void DoDataReceived(object s, EventArgs e)
        {
            ...
            {
                //_CellDynData = new StringBuilder();
                data = Port.ReadExisting();
                ...
                else if (data.Contains("L|1")||data.Contains("1H") || data.Contains("R|"))
                {
                    _CellDynData.Append(data);
                    CellDynRubyData = _CellDynData.ToString();

                    Port.Write(new byte[] { 0x06 }, 0, 1);
                    string[] s1 = { "L|1|N" };


                    string[] cont1 = CellDynRubyData.Split(s1, StringSplitOptions.RemoveEmptyEntries);//, StringSplitOptions.RemoveEmptyEntries
        ...
        private void UpdateInterfceData(object s, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dv.Table.Rows.Count; i++)
                {
                    ...
                    objInterfaced.MethodID = "0124";//hard Coded Cell Dyn ruby method id
                    DataView dv_new = objInterfaced.GetAllOra(2);
                    if (dv_new.Count > 0)
                    {
                        objConnection.OraTransaction_Begin();
                        if (objInterfaced.updateinterfaced())
                        {
    ...
    ToString();
        ...
        private void FillArchData(string data)
        {
            try
            {
                ...
                if (data.Contains("^^F"))
                {
                    ...
                    for (int i = 0; i < SplitData.GetUpperBound(0); i++)
                    {
                        if (SplitData[i].Contains("1|") && (SplitData[i].Contains("^Z") || SplitData[i].Contains("^E")))
                        {
                            MSplitData = SplitData[i].Split(Del1, StringSplitOptions.RemoveEmptyEntries);
        ...
        private void Writependingtest()
        {
            ...
            {
                //string command = "1H|\\^&\r";
                sb_orders.Append("1H|\\^&\r");
                for (int i = 1; i <= dv.Count; i++)
                {
                    sb_orders.Append( "P|" + i.ToString() + "||" + dv[i - 1]["prno"].ToString().Replace("-", "") + "||" + dv[i - 1]["patientname"].ToString().Trim() + "|||" + dv[i - 1]["psex"].ToString().Substring(0, 1).Trim() + "|||||" + dv[i - 1]["referredby"].ToString().Trim() + "||" + dv[i - 1]["PatientWardInfo"].ToString().Replace(" ", "") + "^327-56-9567\r");
                    sb_orders.Append("O|1|" + Convert.ToInt32(dv[i - 1]["labid"].ToString().Substring(dv[i - 1]["labid"].ToString().Length - 7, 7)).ToString().Trim() + "||^^^CBC^1^0|||||||||||Patient^Human\r");
                    DSerialNos += dv[i - 1]["DSerialno"].ToString().Trim() + ",";
                }
                dv.Dispose();

                sb_orders.Append("L|1|N\r" + Convert.ToChar(3));

                string command = sb_orders.ToString();
                sb_orders = null;
                ASCIIEncoding encoding = new ASCIIEncoding();
