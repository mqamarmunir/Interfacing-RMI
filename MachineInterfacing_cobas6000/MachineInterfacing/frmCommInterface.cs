...
namespace MachineInterfacing
{
    public partial class frmCommInterface : Form
    {
        ...
        private void DoDataReceived(object s, EventArgs e)
        {
            ...
            #region "cobas6000"
            if (clsSharedVariable.InstrumentName.Equals("cobas6000"))
            {
                data = Port.ReadExisting();
                Port.Write(new byte[] { 0x06 }, 0, 1);
                if (data.Length > 0)
        ...
        private void InsertBooking(string Msg)
        {
            ...
            try
            {
                ...
                #region "Cobas E411"
                if (clsSharedVariable.InstrumentID.Equals("17"))//cobas e 411
                {
                    string[] Delimeter = { "," };
            ...
            for (int i = 0; i <= abc.GetUpperBound(0); i++)
            {
                ...
                for (int j = 0; j < def.GetUpperBound(0); j++)
                {
                    ...
                    else if (def[j].Contains("R|"))
                    {
                        ...
                        string pars = labid + "," + Regex.Match(attribcode, "\\d+").Value.ToString() + "," + System.DateTime.Now.ToString("dd/MM/yyyy hh:mm") + "," + attribresult;
                        InsertBooking(pars);
                    }
